...
for invariant in (
    ...
    "dre-device-authorizer",
    "dre-password-resetter",
    'serviceAccountName:"dre-admin"',
    'automountServiceAccountToken:false',
    'activeDeadlineSeconds:300',
    'command:["/bin/sleep"],args:["300"]',
    'secretName:"dre-postgres-admin"',
    'Pod administrativo efêmero preexistente',
    'Pod administrativo efêmero permaneceu após o provisionamento',
    'Pod administrativo efêmero permaneceu após a autorização do aparelho',
    'Pod administrativo efêmero permaneceu após o reset da senha',
    'provisioner_cleanup:$cleanup',
    'authorizer_cleanup:$cleanup',
    'resetter_cleanup:$cleanup',
    'platform.servidor.local/deployment-gate=accounts-cleanup-failed',
    'platform.servidor.local/deployment-gate=device-authorization-cleanup-failed',
    'platform.servidor.local/deployment-gate=password-reset-cleanup-failed',
    'mode:"reconcile-existing"',
    'mode:"authorize-device"',
    'mode:"reset-password"',
    ...
    "[HOUSEHOLD_ID PRIMARY_USER_ID SECONDARY_USER_ID]",
    "foreign_login_conflicts",
    "foreign_family_users",
    "audit_user_creates",
    "authorized_device_rows",
    "audit_device_authorizations",
    "audit_password_resets",
    "active_sessions",
    ...
    "/app/dre-admin-cli reset-password",
    "--passwords-stdin",
    "--password-stdin",
    "atomic:true",
    "ROLLBACK;",
    "skipped_existing",
    "release_id:$release_id,pod:$pod,checks:.",
    ...
    "cleanup-restore",
    "restore-cleanup",
    'metadata.labels["dre.familiar/operation-id"] == $source_operation_id',
    "cleanup e restore falho exigem IDs de operação distintos",
    "restore diagnostics",
    "init container do restore terminou com erro",
    "timeout aguardando PostgreSQL restaurado ficar Ready",
    'diagnostics:{pod:$pod,logs:$logs}',
    '.spec.persistentVolumeReclaimPolicy == "Delete"',
    "recibo não comprova restore descartável falho",
    "PVC Pending não foi produzido por falha de materialização",
    "PVC e eventual PV preservados pelo restore falho foram removidos",
    "validate-release",
    "cleanup-validation",
    ...
    '20-database-access.yaml "$old_image" 2',
    '20-database-access.yaml "$new_image" 2',
    '00-platform.yaml "$old_image" 1',
    '00-platform.yaml "$new_image" 1',
    "require_postgres_upgrade_backup",
    "'.generated_at_epoch' \"$plan\"",
    "'.current_release' \"$plan\"",
    "'.protected_fingerprint' \"$plan\"",
    "only_postgres_digest_change:true",
    ...
    "/tmp/pgbackrest/log/dre-check.log",
    "/tmp/pgbackrest/log/dre-archive-push-async.log",
    "pgbackrest_info_code",
    "diagnóstico de produção exige primeiro deploy revertido com PVC preservado",
    "require_diagnostic_exact_names",
    "recibo não comprova rollback do primeiro deploy",
    "diagnóstico exige falha anterior às migrations",
    "namespace_project",
    "require_failed_first_deploy_recovery_state",
    "production_recovery_immutable_fingerprint",
    "pgbackrest_check:\"passed\"",
...
for invariant in (
    ...
    '(.read_only | index("diagnose-password-reset")) != null',
    '(.mutations | index("reconcile-accounts")) != null',
    '(.mutations | index("authorize-device")) != null',
    '(.mutations | index("reset-password")) != null',
    "--as=dre-untrusted",
    ...
    "inventário RBAC/admission DRE diverge da fundação fechada",
...
if 'verify "$release_id"' in active_state:
    fail("bootstrap DRE não deve chamar verify completo dentro do gate de release ativa")
...
for invariant in (
    ...
    "Bootstrap remoto não retornou o atestado final esperado.",
