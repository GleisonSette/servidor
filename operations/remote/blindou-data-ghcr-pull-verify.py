#!/usr/bin/env python3
"""Baixa e valida a imagem PostgreSQL privada sem persistir credenciais."""
...
REPOSITORY = "gleisonsette/blindou-postgres"
...
class RegistryClient:
    ...
    def _registry_request(self, repository: str, path: str, accept: str | None = None):
        ...
        headers = {"User-Agent": "blindou-datactl/1"}
        if accept:
...
def validate_output(path: Path) -> None:
    allowed_root = Path("/var/lib/blindou-data/pull-proofs").resolve()
...
def parse_args() -> argparse.Namespace:
    ...
    match = IMAGE_PATTERN.fullmatch(args.image)
    if not match:
        fail("referência GHCR fora do repositório PostgreSQL aprovado")
    args.index_digest = match.group(1)
    return args
...
def main() -> None:
    ...
    total_bytes = 0
    total_blobs = 0
    with tempfile.TemporaryDirectory(prefix="blindou-data-pull.", dir="/var/tmp") as temporary:
        temporary_root = Path(temporary)
        index = client.manifest(REPOSITORY, args.index_digest)
        platform_digest = select_linux_amd64_manifest(index)
        if platform_digest is None:
            platform_digest = args.index_digest
            platform = index
        else:
            platform = client.manifest(REPOSITORY, platform_digest)
        blobs = manifest_blobs(platform)
        for position, digest in enumerate(blobs):
            destination = temporary_root / f"postgres-{position:03d}-{digest[7:23]}.blob"
    ...
    lines = [
        ...
        f"blobs={total_blobs}",
        f"bytes={total_bytes}",
        f"postgres_image={args.image}",
        f"postgres_platform_digest={platform_digest}",
        f"postgres_blobs={total_blobs}",
        f"postgres_bytes={total_bytes}",
    ]
    args.output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    os.chmod(args.output, 0o600)
    print(
        "blindou_data_pull=passed "
        f"release_id={args.release_id} images=1 blobs={total_blobs} bytes={total_bytes}"
    )
